...
din = 0; #10; din = 1; #10; din = 1; #10; din = 0; #10;
din = 1; #10; din = 1; #10; din = 0; #10; din = 1; #10;
```
**Expected:** `dout` shows input pattern delayed by 12 clock cycles.
...
```
**Expected output (addr → data):** `0→00a, 1→014, 2→01e, 3→028, 4→032, 5→028, 6→01e, 7→014`
...
### 4. Half Adder
```
A=0,B=0 → Sum=0,C=0 | A=0,B=1 → Sum=1,C=0
A=1,B=0 → Sum=1,C=0 | A=1,B=1 → Sum=0,C=1
```
...
### 6. Booth Encoder
```
x1=0,x0=0 → y=0 | x1=0,x0=1 → y=1
x1=1,x0=0 → y=2 | x1=1,x0=1 → y=0
```

### 7. XOR Gate
```
a=0,b=0 → y=0 | a=0,b=1 → y=1
a=1,b=0 → y=1 | a=1,b=1 → y=0
```

### 8. MUX (2-to-1)
```
a=0,b=1,sel=0 → y=0 (selects a)
a=1,b=0,sel=1 → y=0 (selects b)
```
...
### 10. D Flip-Flop (Pipeline Register)
```verilog
clk=0; rst=1; en=0; d=0;
#10 rst=0; en=1; d=1; #10;
d=0; #10; en=0; d=1; #10;  // holds when en=0
en=1; d=1; #10;             // updates when en=1
```
**Expected:** `q` follows `d` only when `en=1`; holds value when `en=0`.
